#!/usr/bin/env python3
"""
Sentence-BERT 기반 시맨틱 검색 RAG 시스템
TF-IDF를 Sentence-BERT 임베딩으로 완전 교체
"""
...
class SemanticEnhancedRAG:
    def __init__(self, data_folder="./data", similarity_threshold=0.3):
        self.data_folder = data_folder
        self.docs = []
        self.metainfo = []
        
        # Sentence-BERT 모델 설정 (다국어 지원, 더 가벼운 모델)
        self.model_name = "paraphrase-multilingual-MiniLM-L12-v2"  # 가벼운 다국어 모델
    ...
    def load_sentence_model(self):
        """Sentence-BERT 모델 로드"""
        try:
            logger.info(f"Sentence-BERT 모델 로드 중: {self.model_name}")
            ...
            # 폴백: 다국어 모델 사용
            try:
                logger.info("폴백: 다국어 Sentence-BERT 모델 사용")
                self.model_name = "paraphrase-multilingual-MiniLM-L12-v2"
                self.sentence_model = SentenceTransformer(self.model_name)
                logger.info("다국어 모델 로드 완료")
            except Exception as e2:
                logger.error(f"폴백 모델도 로드 실패: {e2}")
                raise Exception("Sentence-BERT 모델을 로드할 수 없습니다.")
    ...
    def initialize(self):
        """시스템 초기화"""
        try:
            logger.info("시맨틱 RAG 시스템 초기화 시작...")
            ...
            if self.docs:
                ...
                logger.info("시맨틱 RAG 시스템 초기화 완료!")
            else:
    ...
    def build_embeddings(self):
        ...
        if os.path.exists(embeddings_path):
            try:
                logger.info("기존 임베딩 파일 로드 중...")
                with open(embeddings_path, 'rb') as f:
                    saved_data = pickle.load(f)
                    if len(saved_data['embeddings']) == len(self.docs):
                        self.doc_embeddings = saved_data['embeddings']
                        logger.info("기존 임베딩 파일 로드 완료")
                        return
                    else:
                        logger.info("문서 수가 변경됨. 새로운 임베딩 생성...")
            except Exception as e:
                logger.warning(f"기존 임베딩 로드 실패: {e}. 새로운 임베딩 생성...")

        # 새로운 임베딩 생성
        logger.info("문서 임베딩 생성 중... (시간이 걸릴 수 있습니다)")
        ...
        try:
            ...
            logger.info(f"임베딩 생성 완료: {self.doc_embeddings.shape}")
            
        except Exception as e:
            logger.error(f"임베딩 생성 실패: {e}")
            raise

    def semantic_search(self, question, top_k=6):
        """시맨틱 검색 - Sentence-BERT 기반"""
        ...
        try:
            ...
            logger.info(f"검색 결과: {len(retrieved)}개 문서 (최고 유사도: {max(similarities):.3f})")
            return retrieved, sources
            
        except Exception as e:
            logger.error(f"시맨틱 검색 오류: {e}")
            return [], []
...
위 문서 정보를 바탕으로 질문에 답해주세요."""
    ...
    def generate_answer(self, question):
        ...
        try:
            ...
            # 시맨틱 검색
            retrieved_docs, sources = self.semantic_search(question, top_k=6)
    ...
    def health_check(self):
        ...
        return {
            ...
            "embeddings_ready": self.doc_embeddings is not None,
            "openai_enabled": self.use_openai,
            "model_name": self.gpt_model if self.use_openai else "fallback-mode",
            "sentence_model": self.model_name if self.sentence_model else "not_loaded",
            "version": "semantic_enhanced_rag_system",
            "features": "sentence_bert_search+gpt_generation+fallback",
            "search_algorithm": "sentence-bert_semantic_similarity"
        }